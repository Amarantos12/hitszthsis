...
\begin{figure}[!ht]
  ...
  \begin{minipage}{\textwidth}
    ...
    \centering
    % 迷你页面内容居中
    \subfigure[BlueView P900 声纳图像\label{fig:噪声示意图1}]{\includegraphics[width=0.48\textwidth]{figures/噪声示意图1.png}}
    % 定义第一个子图，标题为“图像1的标题”，插入图片 figures/1.jpg，宽度为文本宽度的 45%
    \hspace{0.5em}
    % 在两个子图之间添加 2em 的水平间距
    \subfigure[Oculus M1200d 声纳图像\label{fig:噪声示意图2}]{\includegraphics[width=0.48\textwidth]{figures/噪声示意图2.png}}
    % 定义第二个子图，标题为“图像2的标题”，插入图片 figures/2.jpg，宽度为文本宽度的 45%
  \end{minipage}
  % \vspace{0.2em}
  % 在图形与下方内容之间添加 0.2em 的垂直间距
  \caption{前视声纳图像噪声示意图}
  \label{fig:噪声示意图} % 为整个图形添加标签
\end{figure}
